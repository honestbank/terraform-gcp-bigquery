...
// creating an external table over a Google Sheets tab, with the schema detected from the header row
module "bigquery_external_sheet_table" {
  #checkov:skip=CKV_GCP_121:Deletion protection is not needed for test resources.
  source = "../../modules/gcp_bigquery_external_sheet_table"
  // dataset id that this table will be created in
  dataset_id = module.bigquery_dataset.id
  // name of this table
  name = "external_sheet_table"
  // description of the table
  description = "external table over a Google Sheets tab"
  // the bare spreadsheet ID, taken from the spreadsheet URL
  spreadsheet_id = var.test_spreadsheet_id
  // the tab and cell range to read; leave it null to read the first tab
  range = var.test_range
  // the first row holds the column headers
  skip_leading_rows = 1
  // let BigQuery detect the column names and types from the header row
  autodetect = true
  // protect terraform from deleting the resource, set to false in this example because the test will destroy it
